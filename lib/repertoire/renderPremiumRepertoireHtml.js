...
function normalizeNotes(item) {
  return compactText(item?.notes || item?.observation || item?.observacao || item?.observacoes || '');
}
...
const SECTION_ORDER = ['antesala', 'cortejo', 'cerimonia', 'saida', 'receptivo', 'custom'];

const SECTION_DEFINITION = {
  antesala: {
    title: 'Antessala',
    icon: 'antessala',
    allowDescriptiveContent: true,
...
function normalizeSectionKey(value) {
  ...
  if (
    ...
    normalized === 'customeventsong' ||
    normalized === 'customeventmeta' ||
    normalized === 'customsong' ||
    normalized === 'customsongs' ||
    normalized === 'musicaespecifica' ||
...
function buildItemDedupeKey(section, item = {}) {
  return [
    section,
    item?.id || '',
...
function groupItemsBySection(payload = {}) {
  ...
  const pushItem = (section, item) => {
    const normalizedSection = normalizeSectionKey(section || item?.section);
    if (!SECTION_ORDER.includes(normalizedSection)) return;

    const dedupeKey = buildItemDedupeKey(normalizedSection, item);
    if (seen.has(dedupeKey)) return;

    seen.add(dedupeKey);
    grouped[normalizedSection].push(item);
  };
  ...
  const sectionSources = [
    ...
    ['custom', payload?.customSongs],
  ];

  sectionSources.forEach(([section, items]) => {
    if (!Array.isArray(items)) return;
    items.forEach((item) => pushItem(section, item));
  });

  [payload?.items, payload?.songs, payload?.raw?.items]
    .filter(Array.isArray)
    .forEach((items) => {
      items.forEach((item) => pushItem(item?.section, item));
    });
...
function renderSimpleListSection(title, iconName, items = []) {
  const validItems = normalizeCustomList(items);
...
export function renderPremiumRepertoireHtml(payload = {}) {
  ...
  const isCustomEvent = Boolean(payload?.isCustomEvent);
  const eventTypeLabel = compactText(payload?.eventTypeLabel || 'EVENTO');
  const selectedStyles = normalizeCustomList(payload?.selectedStyles || payload?.selected_styles);
  const preferredArtists = normalizeCustomList(payload?.preferredArtists || payload?.preferred_artists);

  const dateTimeLine = [eventDate, eventTime ? `às ${eventTime}` : ''].filter(Boolean).join(' ');
  const repertoireTitle = isCustomEvent
    ? `REPERTÓRIO — ${eventTypeLabel || 'EVENTO'}`
...
<head>
  <meta charset="UTF-8" />
  <title>${escapeHtml(repertoireTitle)}</title>
  <style>
    @page {
      size: A4;
...
<body>
  <main class="page">
    <section class="hero-card">
      ...
      <h1 class="title">${escapeHtml(repertoireTitle)}</h1>
    ...
    ${SECTION_ORDER.map((section) => {
      ...
      return renderSection(config.title, config.icon, groupedSections[section], {
        ...
        allowDescriptiveContent: Boolean(config.allowDescriptiveContent),
        showConfigFields: Boolean(config.showConfigFields),
      });
    }).join('')}
    ...
    ${
      isCustomEvent
        ? `
          ${renderSimpleListSection('Estilos musicais desejados', 'music', selectedStyles)}
          ${renderSimpleListSection('Artistas desejados', 'bride', preferredArtists)}
        `
        : ''
    }
