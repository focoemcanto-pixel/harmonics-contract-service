...
function toRepertoirePayload({
  ...
  allItems.forEach((item) => {
    ...
    if (normalizedSection === 'custom') sections.custom.push(item);
  });
  ...
  const selectedStyles = toArraySafe(
    firstFilled(
      base?.selected_styles,
      base?.selectedStyles,
      deepFind(rawPayload, ['selected_styles', 'selectedStyles']),
      deepFind(presetJson, ['selected_styles', 'selectedStyles'])
    )
  );

  const preferredArtists = toArraySafe(
    firstFilled(
      base?.preferred_artists,
      base?.preferredArtists,
      deepFind(rawPayload, ['preferred_artists', 'preferredArtists']),
      deepFind(presetJson, ['preferred_artists', 'preferredArtists'])
    )
  );

  const customSongs = sections.custom.map((item) => ({
    id: item?.id || null,
    title: firstFilled(item?.songName, item?.referenceTitle, item?.title),
    songName: firstFilled(item?.songName, item?.title),
    referenceTitle: firstFilled(item?.referenceTitle),
    referenceLink: firstFilled(item?.referenceLink),
    notes: firstFilled(item?.notes),
    artists: firstFilled(item?.artists),
    genres: firstFilled(item?.genres),
    duration: firstFilled(item?.duration),
    order: Number(item?.order ?? 0),
    section: 'custom',
  }));

  const eventTypeRaw = firstFilled(
    base?.event_type,
    base?.eventType,
    base?.tipo_evento,
    base?.tipoEvento,
    deepFind(rawPayload, ['event.type', 'event.eventType', 'event_type', 'eventType', 'tipo_evento']),
    deepFind(presetJson, ['event.type', 'event.eventType', 'event_type', 'eventType', 'tipo_evento'])
  );

  const normalizedEventType = String(eventTypeRaw || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase();

  let eventTypeLabel = 'EVENTO';
  if (normalizedEventType.includes('cha')) eventTypeLabel = 'CHÁ';
  else if (normalizedEventType.includes('show')) eventTypeLabel = 'SHOW';
  else if (normalizedEventType.includes('receptivo')) eventTypeLabel = 'RECEPTIVO';

  const hasCustomTypeKeyword = /(cha|show|receptivo|corporativo|custom)/.test(normalizedEventType);
  const isCustomEvent =
  ...
  safeLog({
    ...
    hasPreferredArtists: preferredArtists.length > 0,
  });
