...
function toRepertoirePayload({ configRow, itemRows, fallbackRow }) {
  ...
  const normalizeItem = (item) => ({
    title: item?.song_name || item?.reference_title || 'Música sem título',
  ...
  const allItems = normalizedItems.length ? normalizedItems : fallbackSongs;
  const sections = {
    cortejo: [],
    cerimonia: [],
    saida: [],
  ...
  sections.cortejo.sort((a, b) => Number(a.order ?? 0) - Number(b.order ?? 0));
