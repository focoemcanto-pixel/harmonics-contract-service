...
export async function generateGoogleContract({
  ...
  const copied = await drive.files.copy({
    ...
    requestBody: {
      ...
      parents: [monthFolderId], // Pasta onde o arquivo será salvo
    },
  ...
  try {
    ...
    console.log('[googleContractGenerator] PDF gerado com sucesso. Tamanho do buffer:', pdfBuffer.length);

    if (!pdfBuffer || pdfBuffer.length === 0) {
      throw new Error('O PDF não foi gerado corretamente. Buffer vazio.');
    }
  } catch (error) {
  ...
  try {
    pdfCreated = await drive.files.create({
      requestBody: {
        ...
        parents: [monthFolderId], // A pasta onde o arquivo será salvo
      },
      media: {
        mimeType: 'application/pdf',
        body: pdfBuffer,  // O conteúdo do PDF
      },
    ...
    console.log('[googleContractGenerator] PDF salvo com sucesso. ID:', pdfCreated.data.id);
  } catch (error) {
