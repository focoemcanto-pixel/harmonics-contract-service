...
async function listExistingPdfFilesByName(drive, parentFolderId, fileName) {
...
async function deleteFileSafe(drive, fileId, logLabel) {
  if (!fileId) return;
  ...
  try {
    ...
    console.log(`[googleContractGenerator] ${logLabel}`, { fileId });
  } catch (error) {
    console.warn(`[googleContractGenerator] falha ao ${logLabel}`, {
      fileId,
...
export async function generateGoogleContract({
  ...
  const safeContractName = sanitizeFileName(contractName);
  const pdfFileName = `${safeContractName}.pdf`;

  // Limpa PDFs antigos com o mesmo nome antes de criar o novo
  try {
    const existingPdfFiles = await listExistingPdfFilesByName(
      drive,
      monthFolderId,
      pdfFileName
    );

    for (const file of existingPdfFiles) {
      console.log('[googleContractGenerator] removendo PDF antigo', {
        fileId: file.id,
        name: file.name,
      });

      await deleteFileSafe(drive, file.id, 'remover PDF antigo');
    }
  } catch (error) {
    console.warn('[googleContractGenerator] erro ao procurar/remover PDFs antigos', {
      pdfFileName,
      message: error?.message,
    });
  }
  ...
  try {
    ...
    // Sempre tenta remover o DOC temporário ao final do fluxo
    await deleteFileSafe(drive, docId, 'remover DOC temporário');
  }
}
